...
export default function ViewRulesForm() {
  ...
  const [searchAdjective, setSearchAdjective] = useState('');
  const [searchLawClause, setSearchLawClause] = useState('');
  ...
  const filteredMembers = members.filter((member) => {
    ...
    const governorateMatch = searchAdjective ? member.c.toLowerCase().includes(searchAdjective.toLowerCase()) : true;
    const jobMatch = searchLawClause ? member.d.toLowerCase().includes(searchLawClause.toLowerCase()) : true;
  ...
  <Box sx={{ display: "flex" }}>
    ...
    <Box component="main" sx={{ flexGrow: 1, p: 3, overflow: "auto" }}>
      ...
      <div style={{
        ...
        marginTop: "15px",
        display: "flex" }}>
        <h1>القوانين المصدره من العضو</h1>
      </div>
      <div style={{width:"fit-content"}}>
        <Link style={{textDecoration:"none"}} to='../AddRules'>
        <Button sx={{
          ...
          display:"block",
          marginBottom:"25px",
        }} className='addMember' variant="contained" >اضافه القوانين</Button>
      </Link>
      </div>
      ...
      <TableContainer component={Paper} sx={{ marginBottom: "2%" }}>
        <Table>
            ...
            <TableRow  sx={{ backgroundColor: "#dc1f27" }}>
              ...
              <TableCell sx={{ color: "#fff" }}>الصفه </TableCell>
              <TableCell sx={{ color: "#fff" }}> بند القانون</TableCell>
              <TableCell sx={{ color: "#fff" }}>  تاريخ التقديم </TableCell>

              <TableCell sx={{ color: "#fff" }}>الحاله </TableCell>
